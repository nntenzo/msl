...
class Site_settingsService:
    ...
    async def get_list(
        ...
        try:
            query = select(Site_settings)
            count_query = select(func.count(Site_settings.id))
            
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Site_settings, field):
                        query = query.where(getattr(Site_settings, field) == value)
                        count_query = count_query.where(getattr(Site_settings, field) == value)
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Site_settings, field_name):
                        query = query.order_by(getattr(Site_settings, field_name).desc())
                else:
                    if hasattr(Site_settings, sort):
                        query = query.order_by(getattr(Site_settings, sort))
            else:
                query = query.order_by(Site_settings.id.desc())
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Site_settings]:
        ...
        try:
            ...
            result = await self.db.execute(
                select(Site_settings).where(getattr(Site_settings, field_name) == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
    ...
    async def list_by_field(
        ...
        try:
            ...
            result = await self.db.execute(
                ...
                .where(getattr(Site_settings, field_name) == field_value)
                .offset(skip)
                .limit(limit)
