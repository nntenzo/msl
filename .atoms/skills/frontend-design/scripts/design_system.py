...
class DesignSystemGenerator:
    ...
    def generate(self, query: str, project_name: str = None) -> dict:
        ...
        brand_results = self._extract_results(search_results.get("brand", {}))
            ...
            "reference_brands": [
                {
                    ...
                    "heading_font": br.get("Heading_Font", ""),
                    "design_md_path": br.get("Design_MD_Path", ""),
                    "key_characteristics": br.get("Key_Characteristics", "")[:200]
                }
                for br in brand_results[:2]
            ]
        }
...
def format_ascii_box(design_system: dict) -> str:
    ...
    ref_brands = design_system.get("reference_brands", [])
    if ref_brands:
        lines.append("|  REFERENCE BRANDS (Real-World Inspiration):".ljust(BOX_WIDTH) + "|")
        for rb in ref_brands:
            lines.append(f"|     {rb['name']} ({rb['category']})".ljust(BOX_WIDTH) + "|")
            lines.append(f"|       Primary: {rb['primary_color']} | Font: {rb['heading_font']}".ljust(BOX_WIDTH) + "|")
            lines.append(f"|       Full spec: {rb['design_md_path']}".ljust(BOX_WIDTH) + "|")
        lines.append("|" + " " * BOX_WIDTH + "|")
...
def format_markdown(design_system: dict) -> str:
    ...
    ref_brands = design_system.get("reference_brands", [])
    if ref_brands:
        lines.append("### Reference Brands")
        lines.append("Real-world brand design systems that align with this recommendation:")
        lines.append("")
        for rb in ref_brands:
            lines.append(f"**{rb['name']}** ({rb['category']})")
            lines.append(f"- Primary: `{rb['primary_color']}` | Font: {rb['heading_font']}")
            lines.append(f"- Full spec: `{rb['design_md_path']}`")
            lines.append("")
